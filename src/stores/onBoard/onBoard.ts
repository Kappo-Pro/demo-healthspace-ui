...
export const saveProfileDetails = createAsyncThunk(
  'saveProfileDetails',
  async ({ id, payload }: { id: string; payload: TProfileData }): Promise<unknown> => {
    try {
...
export const getSuggestedProgram = createAsyncThunk(
  'getSuggestedProgram',
  async ({ current, functionalGoalsIds }: { current: number; functionalGoalsIds: number[] }): Promise<unknown> => {
...
export const savedFunctionalGoal = createAsyncThunk(
  'savedFunctionalGoal',
  async ({ userId, payload }: { userId: string; payload: FunctionalGoalsProps }): Promise<unknown> => {
...
const onBoard = createSlice({
  ...
  reducers: {
    ...
    setFilterFunctionalGoal : (state, action) => {
      state.filterFunctionalGoal = action.payload
    },
...
export const {
  setImage, setPainStatusButton, setSelectedFunctionalGoalTitle, setDisplayedFunctionalGoalTitle,
  setCurrentStep, setModalVisible, setFunctionalModal,setFilterFunctionalGoal,
  setVideoRecordState, setExistingPatientData, setActiveStep, setOnBoardCompletion, setOnBoardRomCompletion
} = onBoard.actions;
