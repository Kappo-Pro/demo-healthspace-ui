...
import { Check } from '@vitalflow-icons/general/check';
import { InfoCircle } from '@vitalflow-icons/general/infoCircle';
import { UntitledIcon } from '@atoms/Icon';
import { patchRomBodyPoints } from '@stores/clinical/rom/customRom';
import { useTypedDispatch, useTypedSelector } from '@stores/index';
import { CustomRomBodyPoints, CustomRomExercise, RomPatientResult } from '@types';
import { Card, Image, Input, List, message, Progress, Tooltip, Flex, Typography } from 'antd';
...
const CustomSummaryContent = (props: ICustomSummaryContent) => {
  const { item } = props;
  const _kinematicsArray = ['flexion', 'extension', 'abduction', 'external rotation', 'internal rotation'];
  ...
  const [editedPostures, setEditedPostures] = useState<{ [key: string]: CustomRomBodyPoints }>({});
  ...
  const handleSave = async (postureId: string) => {
    ...
    const updatedItems = itemData.map((exercise: CustomRomExercise) => {
  ...
  return (
    <div className="posture-card-container">
      {itemData?.map((exercise: CustomRomExercise, idx: number) => {
        ...
        return (
          <Card
            ...
            style={{ margin: 'var(--spacing-2-5)' }}
            size="small"
            title={
              <div className="posture-title-container">
                <Flex className="posture-title" align="center">
                  <Image
                    width={64}
                    height={53}
                ...
                <div className="posture-label-container" style={{ width: user.isPhysioterapist ? "500px" : "415px", paddingLeft: "5px" }}>
                  {labels.map((label) => (
                    <div key={label} className="posture-label">
                      <Flex className="posture-label-text" align="center" justify="center">
                          ...
                          <Tooltip
                            ...
                            title={
                              <span className="text-gray-700 font-semibold !p-2">
                                {label === "wfl"
                                  ? t('patient.progress.omniRom.wflToolTip')
                                  : t('patient.progress.omniRom.normalToolTip')}
                              </span>
                            }
                ...
                <div className="posture-side-labels-container">
                  {labels.map((label) => {
                          ...
                          {editedPostures[exercise?.romProgramexercise?.id] ? (
                            <Flex gap={8}>
                              <Tooltip title={t('Patient.data.omniRom.cancel')}>
                                <div onClick={() => handleCancel(exercise?.romProgramexercise?.id)} style={{ cursor: 'pointer' }}>
                                  <UntitledIcon name="close" size={20} style={{ color: 'var(--color-error-500)' }} aria-label="Cancel edit" />
                                </div>
                              </Tooltip>
                            ...
                            <Tooltip title={t('Patient.data.omniRom.edit')}>
                              <div onClick={() => handleEdit(idx, exercise?.romProgramexercise?.id)}>
                                <Pencil01 color="stroke-gray-600" />
                              </div>
                            </Tooltip>
                          )}
                        </div>
                    ...
                    const scoreColor = () => {
                      ...
                      if (score >= 60 && score < 90) return 'var(--color-warning-400)';
                      if (score >= 0 && score < 60) return 'var(--color-error-500)';
                      return 'var(--color-gray-500)';
                    };
                    return (
                      <div key={label} className="posture-side-labels-section">
